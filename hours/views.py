        ...
        for key, group in df_full:
                ...
                if all(group < nove):
                    ...
                    list_df.append(
                        (
                            key[0],
                            key[1],
                            (group.min() + tres) - group.min(),
                            "Vespertino",
                        )
                    )
                elif all(group > quinze) & all(group < dezeseis):
                    list_df.append(
                        (key[0], key[1], dezoito - group.min(), "Vespertino")
                    )
            ...
            if group.count() == 2:
                if all(group < dozetrinta):
                    list_df.append(
                        (key[0], key[1], group.max() - group.min(), "Matutino")
                    )
                    ...
                    list_df.append(
                        (key[0], key[1], group.max() - group.min(), "Noturno")
                    )
            ...
            if group.count() == 4:
                if bool(group.max() < dozetrinta):
                    ...
                    if ((val_4[0] - val_4[1]) < doismin) and (
                        ...
                        list_df.append(
                            (key[0], key[1], (val_4[0] - val_4[3]), "Noturno")
                        )
                    else:
                        list_df.append(
                            (
    ...
    try:
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                messages.success(request, "Arquivo analisado com sucesso.")
                return handle
        else:
            form = CalculateForm()
    except ValueError as e:
        messages.error(request, "Não encontrado as colunas necessárias.")
